...
lora_weights =[
    'models/berry_bowlXL_left_column_learn_Q_Linear_and_Conv2_alpha1.0_rank16_noxattn/berry_bowlXL_left_column_learn_Q_Linear_and_Conv2_alpha1.0_rank16_noxattn_last.pt'
]

prompts = [
    'A wolf <lora1.0:wolf_plushie> plushy on a skateboard in times square'
]
...
prompts_main = ["A {} on a skateboard in times square", "a {} sculpture made of gold", "A {} in a swimming pool", "a {} holding Lollipop", "A cartoon style {} indoors"]
...
for i_main_o, main_object in enumerate(list(main_objects.keys())):
    ...
    for rank1 in ranks_trained:
        lora_weights = [f'models/{object1}XL_{tuning_model_name}_Linear_and_Conv2_alpha1.0_rank{rank1}_noxattn/{object1}XL_{tuning_model_name}_Linear_and_Conv2_alpha1.0_rank{rank1}_noxattn_last.pt']
                    ...
                    for lora_weight in lora_weights:
                        ...
                        for i in range(len(scales)):
                            for j in range(len(seeds)):
                                idx = i * len(seeds) + j
                                save_path = os.path.join("single_image_generate", f'image_{tuning_model_name}_{main_object}_prompt_{i_prompt}_seed_{seeds[j]}_rank_{rank}.png')
